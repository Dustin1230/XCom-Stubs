...
//holds all existing XGUnit actors - to avoid foreach DynamicActors all over the code
...
{
	if(!XGBattle_SP(XComTacticalGRI(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kBattle).AtBottomOfRunningStateBeginBlock() || XGBattle_SP(XComTacticalGRI(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kBattle).GetAIPlayer().GetSquad() == none){
...
function CacheAllUnits()
{
	local XGUnit kU;

	m_arrAllUnits.Length = 0;
	foreach DynamicActors(class'XGUnit', kU)
	{
		m_arrAllUnits.AddItem(kU);
	}
}
function SetupHelpers()
{
	...
	if (m_kFriendlyChryssalid == none) {
		foreach m_arrAllUnits(kUnit) {
			if ((kUnit.m_eTeam == eTeam_Neutral || kUnit.GetPlayer() == none) && kUnit.GetCharacter().m_eType == ePawnType_Chryssalid)
			{
				if(IsValidAndNeededHelper(kUnit, ePawnType_Chryssalid))
				{
	...
	if (m_kFriendlySectoid == none) {
		foreach m_arrAllUnits(kUnit) {
			if ((kUnit.m_eTeam == eTeam_Neutral || kUnit.GetPlayer() == none) && kUnit.GetCharacter().m_eType == ePawnType_Sectoid)
			{
				if(IsValidAndNeededHelper(kUnit, ePawnType_Sectoid))
				{
	...
	SetTimer(1.0, false, 'ClearCameraLookAts');
}
event Destroyed()
{
	//just in case
...
function XGUnit CreateHelper(EPawnType ePawnType)
{
	...
	kSpawnPt = Spawn(class'XComSpawnPoint_Alien',,,vect(960.0, 960.0, 960.0));
	...
	kUnit.GetPawn().SetFocalPoint(kUnit.GetLocation());
	kUnit.SetUnitHP(99);
	kUnit.m_aCurrentStats[eStat_Defense]=150;
	return kUnit;
}
...
function KillHelper(XGUnit kUnit)
{
	if(kUnit != none)
	{
		XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveLookAt(kUnit.Location);
		XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveKismetLookAt(kUnit.Location);
		if(kUnit.GetPlayer() != none)
		{
			kUnit.GetPlayer().RemoveUnit(kUnit);
		}
		else
		{
			kUnit.Uninit();
...
{
	KillHelper(m_kFriendlyChryssalid);
	KillHelper(m_kFriendlySectoid);
}

function bool IsValidAndNeededHelper(XGUnit kUnit, EPawnType ePawn)
{
	local bool bValid;

	if(ePawn == ePawnType_Chryssalid)
	{
		bValid = (m_kFriendlyChryssalid == none && kUnit.m_eTeam == eTeam_Neutral && kUnit.GetPlayer() != none && kUnit.GetPlayer() == XGBattle_SP(XComTacticalGRI(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kBattle).GetAnimalPlayer() && kUnit.GetCharacter().m_eType == ePawnType_Chryssalid);
	}
	else if (ePawn == ePawnType_Sectoid)
	{
		bValid = (m_kFriendlySectoid == none && kUnit.m_eTeam == eTeam_Neutral && kUnit.GetPlayer() != none && kUnit.GetPlayer() == XGBattle_SP(XComTacticalGRI(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kBattle).GetAnimalPlayer() && kUnit.GetCharacter().m_eType == ePawnType_Sectoid);
	}
	return bValid;
}
function MoveHelpersToTheSky()
{
	if(m_kFriendlySectoid != none && m_kFriendlySectoid.GetLocation().Z < 4800.00){
		MoveHelper(m_kFriendlySectoid, m_kFriendlySectoid.GetLocation() + vect(0.0, 0.0, 4800.0));
	}
	if(m_kFriendlyChryssalid != none && m_kFriendlyChryssalid.GetLocation().Z < 4800.00){
		MoveHelper(m_kFriendlyChryssalid, m_kFriendlyChryssalid.GetLocation() + vect(0.0, 0.0, 4800.0));
	}		
}
function ClearCameraLookAts()
{
	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveLookAt(vect(960.0, 960.0, 960.0));
	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveLookAt(vect(960.0, 960.0, 960.0));
	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveLookAt(m_kFriendlyChryssalid.Location);
	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveLookAt(m_kFriendlySectoid.Location);

	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveKismetLookAt(vect(960.0, 960.0, 960.0));
	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveKismetLookAt(vect(960.0, 960.0, 960.0));
	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveKismetLookAt(m_kFriendlyChryssalid.Location);
	XComGameReplicationInfo(class'Engine'.static.GetCurrentWorldInfo().GRI).m_kCameraManager.RemoveKismetLookAt(m_kFriendlySectoid.Location);
}